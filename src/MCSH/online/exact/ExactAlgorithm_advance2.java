...
public class ExactAlgorithm_advance2 {
...
//    private String pnbMapPath = "pnbmap.txt";
    ...
    public ExactAlgorithm_advance2(int[][] graph, int[] vertexType, int[] edgeType, Map<Integer, Integer> serial_number_correspondence, Map<Integer, double[]> attribute , Adistance adistance) {
        this.graph = graph;
...
//        double minavgdist = this.adistance.cal_subgraph_attr_dist(community,distancemap);
        ...
        ForkJoinTask<Set<Integer>> task = pool.submit(new Kcore_find(pnbMap,this.queryK, this.queryId,new HashSet<>(),this.adistance,distancemap));
        return task.join();
