...
public class KHindex {
    ...
    private KcoreTreeNode kcoreTreerootNode;
    private Map<Integer, List<Integer>> headmap;
//    private int maxlength;
    private int hnsw_minnum = 5000;

    public KHindex(int[][] graph, int[] vertexType, int[] edgeType, Map<Integer,float[]> attribute){
        this.graph = graph;
    ...
    public void build(MetaPath metaPath, Gweight_float gweight,int M,int ef){
        this.buildmetaPath = metaPath;
        String fileMeta = Config.IndexRoot+buildmetaPath.toString();
        newFolder(fileMeta);
        String rootnodepath = fileMeta+"/index";
        this.kcoreTreerootNode = new KcoreTreeNode(rootnodepath,gweight,M,ef);
    ...
    public Set<Integer> search(int queryId, int queryK ,Gweight_float gweight,float precent){
        ...
        KcoreTreeNode treeNode = this.kcoreTreerootNode;
        ...
        Set<Integer> result = treeNode.search(this.attribute.get(queryId),gweight,precent,this.attribute);
    ...
    public void loadIndex(MetaPath metaPath){
        try{
            String fileMeta = Config.IndexRoot+metaPath.toString();
            ...
            this.kcoreTreerootNode = new KcoreTreeNode(rootnodepath);
...
//
//        KHindex aKindex = new KHindex(graph,vertexType,edgeType,attribute);
...
//        Set<Integer> result = aKindex.search(140528,0,gweight, (float) 0.8);
...
//                distancemap.put(nodeid, adistance_float.cal_distance(140528, nodeid));
//            }
//        }
//
