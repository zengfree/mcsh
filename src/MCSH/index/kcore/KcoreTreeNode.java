...
//    private int Coreness ;
    ...
    public KcoreTreeNode(String indexfile) {
    ...
    public void loadNode(Map<String,Integer> integerMap){
        ...
        for (int i = 0; i < integerMap.get(this.indexfile); i++) {
            ...
            KcoreTreeNode kcoreTreeNode = new KcoreTreeNode(childindexfile);
            kcoreTreeNode.loadNode(integerMap);
            this.childrenlist.add(kcoreTreeNode);
        }
    }

    public KcoreTreeNode(String indexfile, Gweight_float gweight,int m0,int ef) {
        this.indexfile = indexfile + ".n2";
        this.indexdatafile = indexfile + "data.txt";
...
//        List<Set<Integer>> childnodes = new ArrayList<>();
        ...
        boolean flag = findDeleteNodes(copyMap,keepSet,coreness,this.nodes);
        ...
        if(flag){
            ...
            for (Set<Integer> set:connectedBlocks){
                ...
                String childindexfile =this.indexfile.replace(".n2","")+"_"+i;
                KcoreTreeNode kcoreTreeNode = new KcoreTreeNode(childindexfile,this.gweight,this.M0,this.ef);
                kcoreTreeNode.build(copyMap,integerMap,coreness+1,set,headMap);
                this.childrenlist.add(kcoreTreeNode);
                i++;
            }
        }
    ...
    public KcoreTreeNode getChildren(int i) {
        return childrenlist.get(i);
    }

    public Set<Integer> search(float[] queryvec,Gweight_float gweight,float precent ,Map<Integer,float[]> integerMap){
        readCorrespond();
        int M = (int) Math.ceil(this.correspond.size()*precent);
        int ef = (int) Math.ceil(M*1.25);
        Set<Integer> result = this.searchIndex(queryvec,gweight,M, ef,integerMap);
        for(KcoreTreeNode child:this.childrenlist){
            Set<Integer> childresult = child.search(queryvec,gweight,precent,integerMap);
    ...
    //返回待删节点
    private boolean findDeleteNodes(Map<Integer,Set<Integer>> pnbMap,Set<Integer> keepSet,int coreness,Set<Integer> result){
...
//            pnbMap.put(curId, new HashSet<>());//clean all the pnbs of curId
    ...
    //从hnsw索引中搜索出结果
    public Set<Integer> searchIndex(float[] queryvec,Gweight_float gweight,int M,int ef,Map<Integer,float[]> integerMap){
...
//            this.readCorrespond();
    ...
    public void savenode(File file){
        ...
        for (KcoreTreeNode node:this.childrenlist) {
            node.savenode(file);
        }
    }
